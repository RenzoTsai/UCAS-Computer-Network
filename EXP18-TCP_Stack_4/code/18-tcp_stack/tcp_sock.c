...
inline void tcp_set_state(struct tcp_sock *tsk, int state)
{
	...
	if (state == TCP_ESTABLISHED){
		pthread_t timer3;
		pthread_create(&timer3, NULL, tcp_cwnd_record_thread, (void *)tsk);
	}
}
...
struct tcp_sock *tcp_sock_accept(struct tcp_sock *tsk)
{
	...
	if ((pop_stack = tcp_sock_accept_dequeue(tsk)) != NULL) {
		...
		pop_stack->nr_state = TCP_OPEN;
		pop_stack->cwnd = 1;
		pop_stack->ssthresh = 16;
		return pop_stack;
	} else {
		return NULL;
	}
...
int delete_send_buffer_entry(struct tcp_sock *tsk, u32 ack) {
